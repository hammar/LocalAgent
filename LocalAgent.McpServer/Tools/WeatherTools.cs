...
/// <summary>
/// Demonstration weather tools using the weather.gov API. 
/// Copied verbatim from C# MCP SDK examples, under MIT license. 
/// Only for illustrative purposes, to be replaced!
/// </summary>
[McpServerToolType]
public sealed class WeatherTools
{
    ...
    public WeatherTools(ILogger<WeatherTools> logger, IHttpClientFactory httpClientFactory)
    {
        _logger = logger;
        ...
        [Description("The US state to get alerts for. Use the 2 letter abbreviation for the state (e.g. NY).")] string state)
    {
        ...
        _logger.LogInformation("Fetching weather alerts for state: {State}", state);
        ...
        _logger.LogInformation("Processing alerts for state: {State}", state);
    ...
    {
        _logger.LogInformation("Fetching weather forecast for location: {Latitude}, {Longitude}", latitude, longitude);
        var client = _httpClientFactory.CreateClient("WeatherApi");
        var pointUrl = string.Create(CultureInfo.InvariantCulture, $"/points/{latitude},{longitude}");

        _logger.LogInformation("Fetching point data from URL: {PointUrl}", pointUrl);
        ...
        _logger.LogInformation("Fetching forecast from URL: {ForecastUrl}", forecastUrl);
